...
model <- left_join(x = gini_final, y = poverty_final, by = "district") %>%
  ...
  left_join(population_final, by = "district") %>%
  left_join(education_final, by = "district") %>%
  left_join(police_killings_final, by = "district") %>%
  left_join(ideology_final, by = "district") %>%
...
fit <- stan_glm(formula = proportion_violent ~ gini_score + poverty + income + population, 
                data = model, 
                refresh = 0)
